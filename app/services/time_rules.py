...
规则（创建与修改时均强制）：
1. 同一环节内 planned_end ≥ planned_start；
2. 环节的 planned_start ≥ 全部前置环节（已填写的）planned_end 的最大值
...
def validate_stage_times(stages: list[RequirementStage]) -> list[str]:
    ...
    for s in stages:
        ...
        # 规则 2：planned_start ≥ 前置环节（已填写）planned_end 的最大值
